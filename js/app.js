...
const RUTES = [
  ...
  { re: /^\/buscar-equip$/,          vista: viewBuscarEquip,    carregar: carregarBuscarEquip },
  ...
  { re: /^\/noticia\/(.+)$/,         vista: viewNoticia,        carregar: carregarNoticia },
  ...
  { re: /^\/amistosos$/,             vista: viewAmistosos,      carregar: carregarAmistosos },
  { re: /^\/amistosos\/nou$/,         vista: viewNouAmistos,     carregar: carregarNouAmistos },
  { re: /^\/amistosos\/(\d+\/(?:local|visitant))$/, vista: viewEditarEquipAMa,
    carregar: carregarEditarEquipAMa },
];
...
function pintar({ conservarFormularis = false } = {}) {
  ...
  /*
    El mode pista és d'una pantalla concreta: sortint-ne, la barra de
    dalt i la de baix han de tornar. Si no, s'anava a qualsevol altra
    pantalla i es quedava sense navegació.
  */
  if (!/^\/arbitre\/partit\//.test(ruta)) document.body.classList.remove('mode-pista');

  App.accions = {};
  cont.innerHTML = def ? def.vista(param) : viewNoTrobat();

  // Els botons de punt bloquejats han de seguir bloquejats després de
  // repintar: el sondeig repinta tot sol cada pocs segons.
  if (typeof aplicarEspera === 'function') aplicarEspera();
...
const ACCIONS_GLOBALS = {
  ...
  /*
    Donar-se de baixa. Dues passes a posta: la primera diu què es perd i
    la segona demana la contrasenya. No es pot desfer, i una sessió oberta
    en un mòbil que no vigiles no ha de bastar per esborrar-ho.
  */
  esborrarCompte() {
    confirmar({
      titol: 'Esborrar el teu compte?',
      text: 'Perdràs l\'accés i tots els teus permisos. La teva fitxa de jugador i els '
          + 'partits que has jugat es queden a la lliga. Això no es pot desfer.',
      confirma: 'Continuar',
      perill: true,
      onOk() {
        const ov = $('#overlay');
        ov.innerHTML = `
          <div class="sheet">
            <h3>Segur del tot?</h3>
            <p>Escriu la teva contrasenya per confirmar que ets tu.</p>
            <div class="field">
              <label for="baixa-clau">Contrasenya</label>
              <input id="baixa-clau" type="password" autocomplete="current-password" />
            </div>
            <p class="ajuda" id="baixa-error" hidden></p>
            <div class="sheet-actions">
              <button class="btn btn-outline" data-tanca="1">Cancel·lar</button>
              <button class="btn btn-danger" data-fer="1">Esborrar el meu compte</button>
            </div>
          </div>`;
        ov.hidden = false;
        $('#baixa-clau').focus();
        ov.onclick = async e => {
          ...
          const error = $('#baixa-error');
          const clau = $('#baixa-clau').value;
          if (!clau) { error.textContent = 'Escriu la contrasenya.'; error.hidden = false; return; }
          e.target.disabled = true;
          try {
            await Store.esborrarElMeuCompte(clau);
...
document.addEventListener('click', e => {
  ...
  /*
    Quin filtre es toca ho diu el botó, no l'adreça. Abans es mirava si
    l'adreça començava per #/partits, i la portada és #/inici: allà els
    botons de categoria de les jornades canviaven el filtre de la
    classificació i la llista es quedava clavada.
  */
  const catEl = e.target.closest('[data-categoria]');
  if (catEl) {
    const id = Number(catEl.dataset.categoria);
    if (catEl.dataset.per === 'calendari') categoriaCalendari = id;
...
function sondejarEnBucle() {
  setInterval(async () => {
    ...
    try {
      const { canvis, llistaNova } = await Store.sondejar();
      // Si ha començat o acabat algun partit, cal tornar a demanar la
      // llista; si només s'han mogut marcadors, amb repintar n'hi ha prou.
      if (llistaNova) await navegar();
      else if (canvis) pintar({ conservarFormularis: true });
    } catch (_) {
...
(async function arrencar() {
  ...
  /* Quan la cua d'arbitratge envia de fons, la pantalla s'ha de posar al
     dia; però no a mig prémer ni amb un diàleg obert, que se'l menjaria. */
  CuaArbitre.enCanviar = id => {
    if (App.ocupat || !$('#overlay').hidden) return;
    // Un partit acabat que s'acaba d'enviar: ara ja hi ha conformitats i
    // acta per demanar, i repintar no les portaria.
    const ruta = location.hash.replace(/^#/, '');
    if (id && ruta === `/arbitre/partit/${id}` && Store.partit(id)?.estat === 'finalitzat') {
      navegar();
      return;
    }
    pintar({ conservarFormularis: true });
  };
  CuaArbitre.engegar();
})();
