/* =========================================================
   ...
   =========================================================
   Cada botó passa per la cua de cua-arbitre.js: el mòbil aplica
   l'acció amb les mateixes regles que el servidor i la pinta, i
   l'envia quan hi ha xarxa. Si tarda, la pantalla ho diu: un
   punt que encara no és al servidor no es pot fer passar per
   registrat.
   ========================================================= */
...
async function carregarArbitrePartit(id) {
  ...
  if (p && p.estat === 'finalitzat' && !CuaArbitre.pendents(id)) {
    try { await Store.carregarTancament(id); } catch (_) { /* l'acta pot esperar */ }
  }
  /*
    Les convocables es demanen també abans de començar: si la cobertura
    cau just després d'iniciar, sense elles no es podria posar la formació.
  */
  if (p && p.estat !== 'finalitzat' && Store.potArbitrar(p)) {
    try {
      await Store.carregarConvocables(id);
    } catch (_) {
      // Si falla, s'arbitra igual amb el marcador de tota la vida.
    }
    // Tot el que caldria per tornar-lo a obrir sense xarxa.
    CuaArbitre.recordarPartit(p, Store.marcadors[p.id], Store.convocables[p.id]);
    Store.recordarArrencada(
      { usuari: Store.usuari, temporada_activa: Store.temporadaActiva },
      Store.regles ? { marca: Store.marca, regles: Store.regles,
                       marcadors_possibles: Store.marcadorsPossibles } : null);
  }
}

/*
  Si hi ha accions que el servidor encara no té, l'àrbitre ho ha de
  saber: pot seguir arbitrant, però no ha de tancar l'app pensant que
  tot ja és a la web.
*/
function avisCuaArbitre(p) {
  const { pendents, avisar } = CuaArbitre.estat(p.id);
  if (!pendents || !avisar) return '';

  App.accions.enviarCua = () => {
    toast('Provant d\'enviar…');
    CuaArbitre.enviar(p.id);
  };

  return `
    <div class="avis-cua" role="status">
      ${icon('wifiOff', 'icon icon-sm')}
      <span><strong>Sense connexió</strong> · ${pendents} ${pendents === 1 ? 'acció per enviar' : 'accions per enviar'}.
      <span class="avis-cua-detall">Pots seguir arbitrant: s'enviaran soles quan torni la cobertura.</span></span>
      <button class="btn btn-outline btn-sm" data-accio="enviarCua">Reintentar</button>
    </div>`;
}
...
function viewArbitrePartit(id) {
  ...
  // Un amistós torna a la llista d'amistosos, no a la de partits assignats.
  const enrere = p.amistos ? '#/amistosos' : '#/arbitre';
  renderTopbar({ titol: 'Control del partit', enrere, accions: false });
  renderTabbar(null);
  ...
  */
  ...
  App.accions = {
    iniciar() {
      confirmar({
        ...
        text: p.amistos
          ? 'És un amistós: el marcador només el veus tu i no compta per a la lliga.'
          : 'El partit passarà a estar EN DIRECTE i el marcador es veurà a voleimasters.cat.',
        confirma: 'Iniciar',
        onOk: () => accio(async () => {
          ...
          */
          ...
          toast(p.amistos ? 'Amistós iniciat' : 'Partit iniciat · en directe a la web');
        }),
      });
    },
    ...
    */
    ...
    finalitzar() {
      confirmar({
        ...
        text: p.amistos
          ? "El resultat quedarà tancat i en tindràs l'acta. No compta per a la lliga."
          : 'El resultat quedarà tancat i comptarà per a la classificació.',
        confirma: 'Finalitzar',
        perill: true,
        onOk: () => accio(async () => {
    ...
    */
    ...
    esborrarAmistos() {
      confirmar({
        titol: 'Esborrar l\'amistós?',
        text: 'Se n\'esborra el marcador, l\'historial i l\'acta. No es pot desfer.',
        confirma: 'Esborrar',
        perill: true,
        async onOk() {
          try {
            await Store.esborrarAmistos(p.id);
            toast('Amistós esborrat');
            location.hash = '#/amistosos';
          } catch (e) {
            toast(e.message);
          }
        },
  ...
  async function restar(costat) {
    ...
    const teDosZeros = p.punts.local === 0 && p.punts.visitant === 0;

    /*
      Es diu com quedaria el marcador, que és el que es mira per decidir.
      Amb el set a 0-0 no hi ha cap punt a treure: el que passa llavors és
      que es reobre el set anterior, i val més dir-ho que ensenyar un -1.
    */
    const comQuedaria = p.punts[costat] > 0
      ? `El marcador quedarà ${costat === 'local' ? p.punts.local - 1 : p.punts.local}`
        + `-${costat === 'visitant' ? p.punts.visitant - 1 : p.punts.visitant}.`
      : (teDosZeros && (p.sets || []).length
          ? 'El set està a 0-0: es tornarà a obrir el set anterior.'
          : 'No hi ha cap punt seu per treure.');

    confirmar({
      titol: 'Restar un punt?',
      text: `Estàs segur de restar un punt a ${equip.nom}? ${comQuedaria}`,
      confirma: 'Restar el punt',
      perill: true,
      onOk: () => accio(() => Store.restarPunt(p.id, costat)),
    });
  }
  ...
  const accioInici = (p.estat === 'programat' && potArbitrar) ? `
    ...
    <div class="note" style="margin-top:12px">
      ...
      <span>${p.amistos
        ? 'És un <strong>amistós</strong>: el marcador i l\'acta només els veus tu, i no compta per a la lliga.'
        : 'En prémer «Iniciar partit», la web mostrarà el partit com a <strong>EN DIRECTE</strong> i el marcador s\'actualitzarà per a tothom.'}</span>
    </div>` : '';
...
/*
     Només abans de començar: un cop el partit roda, qui porta el marcador
     ja el porta, i oferir el codi al mig de la pantalla de punts només fa
     nosa. El títol diu per a qui és: per a qui arbitrarà sense ser àrbitre
     de la lliga.
  */
  const zonaDonarCodi = (p.pot_donar_codi_arbitre && p.estat === 'programat') ? `
    <div class="card card-pad" style="margin-top:16px">
      ${icon('key', 'icon')}
      <h3 style="font-size:16px;font-weight:800;margin-top:8px">Un àrbitre no registrat ha d'arbitrar?</h3>
  ...
  const resum = acabat ? (CuaArbitre.pendents(p.id) ? `
    <div class="note" style="margin-top:16px;border-style:solid">
      ...
      Encara s'ha d'enviar al servidor: no tanquis la sessió. Els comentaris,
      les conformitats i l'acta sortiran aquí quan s'hagi enviat.</span>
    </div>` : `
    <div class="note" style="margin-top:16px;border-style:solid">
      ${icon('checkCircle', 'icon icon-sm')}
      <span>Partit finalitzat amb resultat <strong>${sg.local}–${sg.visitant}</strong>.
      ${p.amistos ? 'És un amistós: no compta per a la lliga.' : 'El resultat ja consta a la classificació.'}</span>
    </div>
    ${bloqueTancament(p)}`) : '';
  ...
  return `
      ...
      <div class="spread">
        <div>
          <div style="font-size:12px;opacity:.85">
            ${p.amistos ? 'Amistós · no compta per a la lliga'
      ...
      <div class="scoreboard">
        ...
        <div class="sb-mid">
          ...
          ${enDirecte && !localEsquerra ? `
            <div class="sb-canviat">${icon('right', 'icon icon-xs')} Local</div>` : ''}
        </div>
    ...
    ${botonsEquipsAMa(p)}
    ${p.pot_esborrar_amistos ? `
      <button class="btn btn-ghost btn-block" data-accio="esborrarAmistos" style="margin-top:16px">
        ${icon('close', 'icon icon-sm')} Esborrar aquest amistós
      </button>` : ''}
    ...
    async desarFormacio(boto) {
      for (const costat of ['local', 'visitant']) {
        if (formacioTriada[costat].length !== 6) {
          toast(`Falten jugadors a ${costat === 'local' ? p.local.nom : p.visitant.nom}`);
          return;
        }
      }
    ...
    return `
      <div class="card card-pad">
        ...
        ${totes.length ? `
          ...
          <p class="ajuda" style="margin-top:10px">
            Toca'ls en l'ordre de rotació. El primer és qui obre a la P1.
          </p>`
          : `<p class="ajuda">Aquest equip encara no té cap jugador fitxat amb dorsal.
...
/* =========================================================
   AMISTOSOS
   =========================================================
   Un partit entre dos equips de la lliga que no compta per a
   res: serveix per aprofitar el marcador, la formació i l'acta.
   No surt a cap llista, no avisa ningú i només el veu qui l'ha
   creat (i el suplent a qui doni un codi).
   ========================================================= */

async function carregarAmistosos() {
  if (!Store.usuari) return;
  await Store.carregarAmistosos();
}

function viewAmistosos() {
  if (!Store.usuari) return viewLogin();

  renderTopbar({ titol: 'Partits amistosos', enrere: '#/partits' });
  renderTabbar('partits');

  const potCrear = Store.potCrearAmistosos();
  const llista = Store.amistosos.map(p => Store.partit(p.id) || p);

  /*
    Esborrar-los des d'aquí i no només des de dins del partit: un amistós a
    mig jugar que no s'acabarà es queda per sempre a la llista, i entrar-hi
    per treure'l és el camí llarg.
  */
  App.accions = {
    esborrarAmistosDeLaLlista(el) {
      const idAmistos = Number(el.dataset.partit);
      const p = llista.find(x => x.id === idAmistos);
      const nom = p ? `${p.local.nom} – ${p.visitant.nom}` : 'aquest amistós';
      confirmar({
        titol: 'Esborrar l\'amistós?',
        text: `${nom}. Se n'esborra el marcador, l'historial i l'acta, encara que `
            + 'estigui a mitges. No es pot desfer.',
        confirma: 'Esborrar',
        perill: true,
        async onOk() {
          try {
            await Store.esborrarAmistos(idAmistos);
            toast('Amistós esborrat');
            pintar();
  ...
  return `
    <div class="page-head">
      <span class="pill pill-yellow">Amistosos</span>
      <h2 class="page-title">Partits amistosos</h2>
      <p class="page-sub">El marcador i l'acta per a un partit que no compta per a la lliga.
      No surt enlloc: només el veus tu.</p>
    </div>

    ${potCrear ? `
      <button class="btn btn-primary btn-block" data-anar="#/amistosos/nou" style="margin-top:6px">
        ${icon('plus', 'icon icon-sm')} Nou amistós
      </button>` : ''}

    <div class="section-title">${icon('calendar', 'icon icon-sm')} Els teus amistosos</div>
    <div class="stack">
      ${llista.length
        ? llista.map(p => `
          <div class="amistos-fila">
            ${filaPartit(p, `#/arbitre/partit/${p.id}`)}
            ${p.creat_per === Store.usuari?.id ? `
              <button class="icon-btn boto-no amistos-treure" data-accio="esborrarAmistosDeLaLlista"
                      data-partit="${p.id}" aria-label="Esborrar l'amistós"
                      title="Esborrar l'amistós">${icon('close')}</button>` : ''}
          </div>`).join('')
        : '<p class="empty">Encara no n\'has fet cap.</p>'}
    </div>`;
}

/** Avui, a l'hora d'aquí: toISOString() donaria el dia d'UTC. */
function avuiLocal() {
  const d = new Date();
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
}

/* ---------- Jugadores escrites a mà ----------
   Un equip que no és de la lliga: el nom i, per a cada jugadora, dorsal,
   nom i les dues marques que mira la regla de pista (dona i menor de 30).
   Sis com a mínim, que són les de pista. */

const JUGADORES_MINIMES = 6;

function jugadoresBuides(n = JUGADORES_MINIMES) {
  return Array.from({ length: n }, () => ({ dorsal: '', nom: '', dona: false, menor30: false }));
}

/** L'editor d'un equip a mà. `clau` distingeix els dos de la mateixa pantalla. */
function editorEquipAMa(clau, equip) {
  return `
    <div class="field">
      <label for="ma-nom-${clau}">Nom de l'equip</label>
      <input id="ma-nom-${clau}" type="text" maxlength="120" value="${esc(equip.nom)}" placeholder="CV Sant Cugat" />
    </div>
    <div class="jugadores-a-ma" data-clau="${clau}">
      <div class="jugadora-a-ma capcal" aria-hidden="true">
        <span>Dorsal</span><span>Nom</span><span></span>
      </div>
      ${equip.jugadores.map((j, i) => `
        <div class="jugadora-a-ma" data-i="${i}">
          <input type="number" inputmode="numeric" min="0" max="199" data-camp="dorsal"
                 value="${esc(String(j.dorsal))}" aria-label="Dorsal" />
          <input type="text" maxlength="120" data-camp="nom" value="${esc(j.nom)}"
                 placeholder="Nom" aria-label="Nom" />
          <button class="icon-btn ghost" data-accio="treureJugadoraAMa" data-clau="${clau}" data-i="${i}"
                  aria-label="Treure-la">${icon('close', 'icon icon-sm')}</button>
          <div class="tria-posicions jugadora-marques">
            <label class="casella"><input type="checkbox" data-camp="dona" ${j.dona ? 'checked' : ''} /> Dona</label>
            <label class="casella"><input type="checkbox" data-camp="menor30" ${j.menor30 ? 'checked' : ''} /> Menor de 30</label>
          </div>
        </div>`).join('')}
    </div>
    <button class="btn btn-outline btn-block" data-accio="afegirJugadoraAMa" data-clau="${clau}" style="margin-top:10px">
      ${icon('plus', 'icon icon-sm')} Afegir jugador
    </button>`;
}

/** Llegeix el que hi ha escrit a la pantalla, abans de repintar-la. */
function llegirEquipAMa(clau, equip) {
  const nom = $(`#ma-nom-${clau}`);
  if (!nom) return;
  equip.nom = nom.value;
  document.querySelectorAll(`.jugadores-a-ma[data-clau="${clau}"] .jugadora-a-ma[data-i]`).forEach(fila => {
    const j = equip.jugadores[Number(fila.dataset.i)];
    if (!j) return;
    j.dorsal = fila.querySelector('[data-camp="dorsal"]').value;
    j.nom = fila.querySelector('[data-camp="nom"]').value;
    j.dona = fila.querySelector('[data-camp="dona"]').checked;
    j.menor30 = fila.querySelector('[data-camp="menor30"]').checked;
  });
}

/** Comprova i converteix per a l'API. Llança un Error amb el que falla. */
function equipAMaPerEnviar(equip, quin) {
  const nom = equip.nom.trim();
  if (!nom) throw new Error(`Posa el nom de l'equip ${quin}`);
  const jugadores = equip.jugadores
    .filter(j => String(j.dorsal).trim() !== '' || j.nom.trim() !== '')
    .map(j => {
      const dorsal = Number(j.dorsal);
      if (!j.nom.trim()) throw new Error(`A ${nom} hi ha un dorsal sense nom`);
      if (String(j.dorsal).trim() === '' || !Number.isInteger(dorsal) || dorsal < 0 || dorsal > 199) {
        throw new Error(`${j.nom.trim()} (${nom}) necessita un dorsal del 0 al 199`);
      }
      return { dorsal, nom: j.nom.trim(), genere: j.dona ? 'dona' : 'home', menor_30: j.menor30 };
    });
  const repetit = jugadores.find((j, i) => jugadores.findIndex(k => k.dorsal === j.dorsal) !== i);
  if (repetit) throw new Error(`A ${nom} el dorsal ${repetit.dorsal} està repetit`);
  if (jugadores.length < JUGADORES_MINIMES) {
    throw new Error(`A ${nom} hi ha d'haver com a mínim ${JUGADORES_MINIMES} jugadors`);
  }
  return { nom, jugadores };
}
...
async function carregarNouAmistos() {
  if (!Store.usuari) return;
  if (!Store.equipsAmistos.length) await Store.carregarEquipsAmistos();
  if (!nouAmistos) {
    const bandol = () => ({ mode: 'lliga', equip_id: '', nom: '', jugadores: jugadoresBuides() });
    nouAmistos = { local: bandol(), visitant: bandol(), data: avuiLocal(), pista: '' };
  }
}

function viewNouAmistos() {
  if (!Store.usuari) return viewLogin();
  renderTopbar({ titol: 'Nou amistós', enrere: '#/amistosos', accions: false });
  renderTabbar(null);
  if (!nouAmistos) return viewNoTrobat();

  const estat = nouAmistos;
  const llegir = () => {
    for (const costat of ['local', 'visitant']) {
      const b = estat[costat];
      if (b.mode === 'lliga') {
        const sel = $(`#am-equip-${costat}`);
        if (sel) b.equip_id = sel.value;
      } else {
        llegirEquipAMa(costat, b);
      }
    }
    if ($('#am-data')) estat.data = $('#am-data').value;
    if ($('#am-pista')) estat.pista = $('#am-pista').value;
  };

  App.accions = {
    modeBandol(el) {
      llegir();
      estat[el.dataset.costat].mode = el.dataset.mode;
      pintar();
    },
    afegirJugadoraAMa(el) {
      llegir();
      estat[el.dataset.clau].jugadores.push(...jugadoresBuides(1));
      pintar();
    },
    treureJugadoraAMa(el) {
      llegir();
      estat[el.dataset.clau].jugadores.splice(Number(el.dataset.i), 1);
      pintar();
    },
    async crearAmistos(boto) {
      llegir();
      const dades = {};
      try {
        for (const costat of ['local', 'visitant']) {
          const b = estat[costat];
          if (b.mode === 'lliga') {
            if (!b.equip_id) throw new Error(`Tria l'equip ${costat}`);
            dades[costat] = { equip_id: Number(b.equip_id) };
          } else {
            dades[costat] = equipAMaPerEnviar(b, costat);
          }
        }
        if (dades.local.equip_id && dades.local.equip_id === dades.visitant.equip_id) {
          throw new Error('Un equip no pot jugar contra ell mateix');
        }
      } catch (e) {
        toast(e.message);
        return;
      }
      if (estat.data) dades.data = estat.data;
      if (estat.pista.trim()) dades.pista = estat.pista.trim();

      boto.disabled = true;
      try {
        const partit = await Store.crearAmistos(dades);
        nouAmistos = null;
        toast('Amistós creat');
        location.hash = `#/arbitre/partit/${partit.id}`;
      } catch (e) {
        toast(e.message);
        boto.disabled = false;
      }
  ...
  const grups = [];
  for (const e of Store.equipsAmistos) {
    let g = grups.find(x => x.id === e.categoria.id);
    if (!g) grups.push(g = { id: e.categoria.id, nom: e.categoria.nom, equips: [] });
    g.equips.push(e);
  }

  const bloc = (costat, titol) => {
    const b = estat[costat];
    return `
      <div class="section-title">${icon('shield', 'icon icon-sm')} ${titol}</div>
      <div class="card card-pad">
        <div class="tria-servei" style="margin-bottom:12px">
          <button class="btn ${b.mode === 'lliga' ? 'btn-primary' : 'btn-outline'}"
                  data-accio="modeBandol" data-costat="${costat}" data-mode="lliga">De la lliga</button>
          <button class="btn ${b.mode === 'ma' ? 'btn-primary' : 'btn-outline'}"
                  data-accio="modeBandol" data-costat="${costat}" data-mode="ma">Escriure'l a mà</button>
        </div>
        ${b.mode === 'lliga' ? `
          <div class="field">
            <label for="am-equip-${costat}">Equip</label>
            <select id="am-equip-${costat}">
              <option value="">— Tria un equip —</option>
              ${grups.map(g => `
                <optgroup label="${esc(g.nom)}">
                  ${g.equips.map(e => `<option value="${e.id}" ${String(e.id) === String(b.equip_id) ? 'selected' : ''}>${esc(e.nom)}</option>`).join('')}
                </optgroup>`).join('')}
            </select>
          </div>
          <p class="ajuda" style="margin:0">Les jugadores surten de la seva plantilla.</p>`
          : editorEquipAMa(costat, b)}
      </div>`;
  };

  return `
    <div class="page-head">
      <span class="pill pill-yellow">Amistós</span>
      <h2 class="page-title">Nou amistós</h2>
      <p class="page-sub">No compta per a la lliga ni surt a cap llista. Cada equip pot ser
      de la lliga o escrit a mà.</p>
    </div>

    ${bloc('local', 'Local')}
    ${bloc('visitant', 'Visitant')}

    <div class="section-title">${icon('calendar', 'icon icon-sm')} Quan i on</div>
    <div class="card card-pad">
      <div class="field">
        <label for="am-data">Dia</label>
        <input id="am-data" type="date" value="${esc(estat.data)}" />
      </div>
      <div class="field" style="margin-bottom:0">
        <label for="am-pista">Pista (opcional)</label>
        <input id="am-pista" type="text" maxlength="150" value="${esc(estat.pista)}" placeholder="On es juga" />
      </div>
    </div>

    <button class="btn btn-primary btn-block" data-accio="crearAmistos" style="margin-top:16px;height:54px">
      Crear i obrir el marcador
    </button>`;
}

/* ---------- Canviar un equip escrit a mà ----------
   Si arriba algú tard o hi ha un nom mal escrit. Qui ja ha sortit a
   pista no es pot treure: l'acta el cita. */

let equipEditant = null;

async function carregarEditarEquipAMa(param) {
  const [id, costat] = String(param).split('/');
  await Store.carregarPartit(id);
  const conv = await Store.carregarConvocables(id, true);
  const p = Store.partit(id);
  const equip = p?.[costat];
  equipEditant = {
    id: Number(id), costat,
    nom: equip?.nom || '',
    jugadores: (conv[costat]?.jugadores || []).map(j => ({
      dorsal: j.dorsal, nom: j.nom, dona: j.genere === 'dona', menor30: j.menor_30 === true,
    })),
  };
}

function viewEditarEquipAMa(param) {
  const [id] = String(param).split('/');
  renderTopbar({ titol: 'Jugadors', enrere: `#/arbitre/partit/${id}`, accions: false });
  renderTabbar(null);
  const e = equipEditant;
  if (!e || e.id !== Number(id)) return viewNoTrobat();

  App.accions = {
    afegirJugadoraAMa() {
      llegirEquipAMa('ed', e);
      e.jugadores.push(...jugadoresBuides(1));
      pintar();
    },
    treureJugadoraAMa(el) {
      llegirEquipAMa('ed', e);
      e.jugadores.splice(Number(el.dataset.i), 1);
      pintar();
    },
    async desarEquipAMa(boto) {
      llegirEquipAMa('ed', e);
      let dades;
      try { dades = equipAMaPerEnviar(e, e.costat); } catch (err) { toast(err.message); return; }
      boto.disabled = true;
      try {
        await Store.desarEquipAMa(e.id, e.costat, dades);
        toast('Equip desat');
        location.hash = `#/arbitre/partit/${e.id}`;
      } catch (err) {
        toast(err.message);
        boto.disabled = false;
      }
  ...
  return `
    <div class="page-head">
      <span class="pill pill-yellow">Amistós</span>
      <h2 class="page-title">${esc(e.nom)}</h2>
      <p class="page-sub">Equip escrit a mà. Pots afegir-hi jugadors o corregir-los;
      qui ja ha jugat en aquest partit no es pot treure.</p>
    </div>
    <div class="card card-pad">${editorEquipAMa('ed', e)}</div>
    <button class="btn btn-primary btn-block" data-accio="desarEquipAMa" style="margin-top:16px;height:54px">
      Desar
    </button>`;
}

/** Els botons per canviar els equips escrits a mà, per a qui ha creat l'amistós. */
function botonsEquipsAMa(p) {
  if (!p.amistos || !p.pot_esborrar_amistos) return '';
  const costats = ['local', 'visitant'].filter(c => p[c]?.extern);
  return costats.map(c => `
    <button class="btn btn-outline btn-block" data-anar="#/amistosos/${p.id}/${c}" style="margin-top:10px">
      ${icon('edit', 'icon icon-sm')} Jugadors de ${esc(p[c].nom)}
    </button>`).join('');
}
