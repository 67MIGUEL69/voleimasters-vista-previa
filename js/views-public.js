...
async function carregarPartits() {
  ...
  /*
    A «Propers» només els que ja tenen dia. Mentre els dos equips no
    l'hagin acordat, el partit existeix però no se sap quan es juga, i
    omplir la llista de partits sense data no ajuda ningú.
  */
  if (vistaPartits === 'propers') llistaPartits = llistaPartits.filter(p => p.data);
...
function viewPartits() {
  ...
  const buit = {
    ...
    propers: 'Encara no hi ha cap partit amb dia acordat.',
  ...
  return `
    ...
    ${esPortada && noticia ? `
      ...
      <button class="card card-pad card-clicable"
              data-anar="#/noticia/${esc(noticia.slug || noticia.id)}">
...
/*
  Els amistosos, a baix de tot i petit: és una eina per a qui porta
  equips o arbitra, no una secció de la lliga. A la resta no li surt.
*/
function enllacAmistosos() {
  if (!Store.usuari || !Store.potCrearAmistosos() || esAppEmpaquetada()) return '';
  return `
    <div class="enllac-discret-lloc">
      <button class="enllac-discret" data-anar="#/amistosos">
        ${icon('whistle', 'icon icon-sm')} Partit amistós
      </button>
    </div>`;
}
      ...
      <details class="card jornada"${j === actual ? ' open' : ''}>
        <summary>
          ...
          <span class="jornada-info">
            ...
            <small>${esc(finestraJornada(llista[0]))}</small>
          </span>
...
function bloqueBuscarEquip() {
  ...
  /*
    Com una opció i no com un títol: amb el text centrat i en negreta, a
    dalt de la pantalla i ocupant tota l'amplada, es llegia com el
    capçal de la secció i ningú no hi tocava.
  */
  return `
    <button class="opcio-accio" data-anar="#/buscar-equip">
      <span class="opcio-accio-icona">${icon('users', 'icon icon-sm')}</span>
      <span class="opcio-accio-text">
        <strong>Busques equip?</strong>
        <span>Demana entrar en un equip de la lliga</span>
      </span>
      ${icon('right', 'icon icon-sm opcio-accio-fletxa')}
    </button>`;
}
...
function viewEquip(id) {
  ...
  const propers = fitxa.partits.filter(p => p.estat !== 'finalitzat').slice(0, 7);
  const ultims = fitxa.partits.filter(p => p.estat === 'finalitzat').slice(-7).reverse();
...
function formulariEquip(fitxa) {
  ...
  return `
    <div class="card card-pad">
      ...
      <div class="field">
        <label for="f-nom">Nom de l'equip</label>
        <input id="f-nom" type="text" maxlength="120" value="${esc(e.nom || '')}" />
        <p class="nota-petita">Surt al marcador, a la classificació i a l'acta.</p>
      </div>
  ...
  return `
    ...
    <div class="stack">
      ${Store.noticies.length ? Store.noticies.map(n => `
        <button class="card card-pad card-clicable noticia-targeta"
                data-anar="#/noticia/${esc(n.slug || n.id)}">
          ...
          <span class="spread" style="margin-top:6px">
            <span class="page-sub">${esc(formatData(String(n.publicada_a).slice(0, 10)))}</span>
            <span class="noticia-llegir">Llegir-la ${icon('right', 'icon icon-sm')}</span>
          </span>
        </button>`).join('')
...
/* ---------- Una notícia sencera ----------
   Abans la llista ho era tot: el títol i l'entradeta, i el text que la
   coordinació hi escrivia no es podia llegir enlloc. */

// Nom propi a posta: views-admin.js ja té un «noticiaOberta» i tots els
// fitxers comparteixen l'àmbit global.
let noticiaLlegint = null;

async function carregarNoticia(clau) {
  noticiaLlegint = await Store.carregarNoticia(clau);
}

function viewNoticia() {
  const n = noticiaLlegint;
  renderTopbar({ titol: 'Notícia', enrere: '#/noticies' });
  renderTabbar('noticies');
  if (!n) return viewNoTrobat();

  // El text es parteix per paràgrafs i s'escapa: el que escriu la
  // coordinació no pot ficar codi a la pàgina de ningú.
  const paragrafs = String(n.cos || '')
    .split(/\n\s*\n/)
    .map(p => p.trim())
    .filter(Boolean)
  ...
  return `
    <article class="noticia-sencera">
      ${n.etiqueta ? `<span class="pill pill-yellow">${esc(n.etiqueta)}</span>` : ''}
      <h2 class="page-title" style="margin-top:10px">${esc(n.titol)}</h2>
      <p class="page-sub">${esc(formatData(String(n.publicada_a).slice(0, 10)))}</p>
      ${n.entradeta ? `<p class="noticia-entradeta">${esc(n.entradeta)}</p>` : ''}
      ${paragrafs ? `<div class="noticia-cos">${paragrafs}</div>` : ''}
    </article>

    <button class="btn btn-outline btn-block" data-anar="#/noticies" style="margin-top:20px">
      ${icon('left', 'icon icon-sm')} Totes les notícies
    </button>`;
}
  ...
  return `
    ...
    <div class="card">
      ...
      ${u ? `
        ...
        <button class="log-item" style="width:100%;color:var(--red)" data-accio="esborrarCompte">
          ${icon('close', 'icon icon-sm')} Esborrar el meu compte
        </button>` : ''}
    </div>
...
function bloqueOrganitzar(p, id) {
  ...
  // Al visitant li surt igualment: ha de poder contestar la proposta del local.
  if (!p.pot_editar_dades && !p.pot_assignar_arbitre && !prop.pot_contestar) return '';
  ...
  return `
    ...
    <div class="card card-pad" style="margin-bottom:14px">
      ...
      ${p.pot_editar_dades ? `
        ...
        `
      : `<p class="ajuda">El dia i la pista els posa l'equip local. Quan proposi un dia,
         aquí el podràs acceptar o rebutjar.</p>`}
