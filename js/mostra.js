...
(function () {
  ...
  /**
   * Qui ha entrat, mana sobre algun dels dos equips del partit?
   *
   * És la versió curta del que fa Capacitats al servidor: prou per
   * decidir quins blocs s'ensenyen a la mostra.
   */
  function manaSobre(partit, rols) {
    if (!sessio) return false;
    return (sessio.permisos || []).some(pe => {
      if (!rols.includes(pe.rol)) return false;
      if (pe.ambit_tipus !== 'equip') return true;   // global, club o competició
      return pe.ambit_id === partit.local.id || pe.ambit_id === partit.visitant.id;
    });
  }
  ...
  window.fetch = async function (url, opcions = {}) {
    ...
    if (cami === '/auth/entrar') {
      ...
      /*
        Si la fotografia porta el perfil sencer d'aquest compte, es fa
        servir aquell: hi van les capacitats, i sense elles Store.pot()
        deia que no a tot i mitja app no sortia.
      */
      sessio = INSTANTANIA['/auth/jo|' + u.email] || {
    ...
    if (detall) {
      ...
      /*
        Aquests dos els calcula el servidor per a cada usuari, i la
        fotografia es fa amb un de sol: venien sempre en fals i el bloc
        d'organitzar no sortia a ningú. Aquí es dedueixen dels permisos
        de qui ha entrat, que és el que fa el servidor de debò.
      */
      amb.pot_editar_dades = p.estat === 'programat'
        && manaSobre(p, ['coord_equip', 'coord_club', 'coord_lliga', 'admin']);
      amb.pot_assignar_arbitre = p.estat !== 'finalitzat'
        && manaSobre(p, ['capita', 'entrenador', 'coord_equip', 'coord_club',
                         'coord_lliga', 'admin']);
    ...
    /*
      A la mostra no s'hi queda cap partit: no hi ha ningú a l'altra
      banda per contestar. La pantalla ho ha de saber sense petar.
    */
    /*
      Els documents del club també depenen de qui ha entrat: es capturen
      amb el correu a la clau, com la fitxa pròpia.
    */
    if (cami === '/clubs/meus') {
      if (!sessio) return error('Cal iniciar sessió.', 401);
      return resposta(INSTANTANIA['/clubs/meus|' + sessio.email] || { clubs: [] });
    }

    /*
      La fitxa pròpia depèn de qui hagi entrat, no de l'adreça: a la
      fotografia es guarda amb el correu a la clau. Qui no en tingui
      —l'administració, per exemple— rep la pantalla que ho explica.
    */
    if (cami === '/jo/pendents') {
      if (!sessio) return error('Cal iniciar sessió.', 401);
      return resposta(INSTANTANIA['/jo/pendents|' + sessio.email] || { pendents: [] });
    }

    if (cami === '/jo/fitxa') {
      if (!sessio) return error('Cal iniciar sessió.', 401);
      return resposta(INSTANTANIA['/jo/fitxa|' + sessio.email]
        || { fitxa: null, pot_editar: false });
    }

    if (/^\/partits\/\d+\/disponibilitat$/.test(cami)) {
      return resposta({ equips: [], persona_id: null });
    }

    if (/^\/partits\/\d+\/propostes$/.test(cami)) {
      return resposta({ propostes: [], pendent: null,
                        pot_contestar: false, pot_proposar: false });
    }
