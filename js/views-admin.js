...
async function carregarAdmin() {
  ...
  if (seccioAdmin === 'clubs') {
    await Store.carregarClubsLliga();
  }
}
...
/* ---------- Clubs ----------
   Quins clubs hi ha i quins equips té cadascun. Els equips es mouen
   d'un club a un altre, o es deixen sense club, i els clubs es creen,
   es reanomenen i s'esborren. Un club esborrat no s'emporta els seus
   equips: queden sense club fins que se'ls en posi un.

   Crear l'equip dins d'un club ho fa la coordinació del club, des de la
   pantalla d'Equips. */

function bloqueClubs() {
  const clubs = Store.clubsLliga;
  const sense = Store.equipsSenseClub;

  const nomDeClub = id => clubs.find(c => c.id === id)?.nom || '';

  function fullDeNom({ titol, text, valor = '', boto, onOk }) {
    const ov = $('#overlay');
    ov.innerHTML = `
      <div class="sheet">
        <h3>${esc(titol)}</h3>
        ${text ? `<p>${esc(text)}</p>` : ''}
        <div class="field">
          <label for="cl-nom">Nom del club</label>
          <input id="cl-nom" type="text" maxlength="120" value="${esc(valor)}"
                 placeholder="CE Exemple" />
        </div>
        <p class="ajuda" id="cl-error" hidden></p>
        <div class="sheet-actions">
          <button class="btn btn-outline" data-tanca="1">Cancel·lar</button>
          <button class="btn btn-primary" data-fer="1">${esc(boto)}</button>
        </div>
      </div>`;
    ov.hidden = false;
    $('#cl-nom').focus();
    ...
    ov.onclick = async e => {
      ...
      if (!nom) { error.textContent = 'Posa-li un nom.'; error.hidden = false; return; }
  ...
  Object.assign(App.accions, {
    nouClub() {
      fullDeNom({
        titol: 'Club nou',
        text: 'Després hi podràs moure els equips que en siguin.',
        boto: 'Crear',
        onOk: async nom => { await Store.crearClub(nom); toast(`${nom} creat`); },
      });
    },

    reanomenarClub(el) {
      const id = Number(el.dataset.club);
      fullDeNom({
        titol: 'Canviar el nom del club',
        valor: nomDeClub(id),
        boto: 'Desar',
        onOk: async nom => { await Store.reanomenarClub(id, nom); toast('Nom canviat'); },
      });
    },
    ...
    esborrarClub(el) {
      ...
      const quants = club?.equips.length || 0;
      confirmar({
        titol: `Esborrar ${club?.nom}?`,
        text: quants
          ? `${quants === 1 ? 'El seu equip quedarà' : `Els seus ${quants} equips quedaran`} sense club `
            + 'fins que els en posis un altre. Els documents privats del club sí que es perden.'
          : 'No té cap equip. Els seus documents privats es perden.',
        confirma: 'Esborrar',
        perill: true,
        async onOk() {
          try {
            const d = await Store.esborrarClub(id);
            toast(d.equips_sense_club
              ? `Club esborrat · ${d.equips_sense_club} ${d.equips_sense_club === 1 ? 'equip queda' : 'equips queden'} sense club`
              : 'Club esborrat');
            pintar();
          } catch (e) { toast(e.message); }
        },
    ...
    moureEquip(el) {
      ...
      const actual = el.dataset.clubActual || '';

      const ov = $('#overlay');
      ov.innerHTML = `
        <div class="sheet">
          <h3>De quin club és ${esc(nom)}?</h3>
          <div class="field">
            <label for="cl-desti">Club</label>
            <select id="cl-desti">
              <option value="" ${actual === '' ? 'selected' : ''}>— Sense club —</option>
              ${clubs.map(c => `<option value="${c.id}" ${String(c.id) === actual ? 'selected' : ''}>${esc(c.nom)}</option>`).join('')}
            </select>
          </div>
          <p class="ajuda">Moure'l canvia qui el pot gestionar: la coordinació del
          club nou, i ja no la de l'anterior.</p>
          <div class="sheet-actions">
            <button class="btn btn-outline" data-tanca="1">Cancel·lar</button>
      ...
      ov.onclick = async e => {
        if (e.target === ov || e.target.dataset.tanca) { ov.hidden = true; ov.innerHTML = ''; return; }
        if (!e.target.dataset.fer) return;
        const valor = $('#cl-desti').value;
        e.target.disabled = true;
        try {
          await Store.moureEquipDeClub(equipId, valor === '' ? null : Number(valor));
          ov.hidden = true; ov.innerHTML = '';
          toast(valor === '' ? `${nom} queda sense club` : `${nom} → ${nomDeClub(Number(valor))}`);
          pintar();
        } catch (err) {
          toast(err.message);
          e.target.disabled = false;
  ...
  const filaEquip = (e, clubId) => `
    <div class="log-item">
      <span>
        ${esc(e.nom)}
        <span class="ajuda">${e.categoria ? esc(e.categoria) : 'Sense grup'}${e.arxivat ? ' · arxivat' : ''}</span>
      </span>
      <button class="btn btn-outline btn-petit" data-accio="moureEquip" style="margin-left:auto"
              data-equip="${e.id}" data-nom="${esc(e.nom)}" data-club-actual="${clubId ?? ''}">
        Moure
      </button>
    </div>`;

  return `
    <button class="btn btn-primary btn-block" data-accio="nouClub" style="margin-bottom:16px">
      ${icon('plus', 'icon icon-sm')} Club nou
    </button>

    ${clubs.map(c => `
      <div class="section-title">
        ${icon('shield', 'icon icon-sm')} ${esc(c.nom)}
        <span class="ajuda" style="margin-left:auto">${c.equips.length} ${c.equips.length === 1 ? 'equip' : 'equips'}</span>
        <button class="icon-btn ghost" data-accio="reanomenarClub" data-club="${c.id}"
                aria-label="Canviar el nom">${icon('edit', 'icon icon-sm')}</button>
        <button class="icon-btn ghost" data-accio="esborrarClub" data-club="${c.id}"
                aria-label="Esborrar el club">${icon('close', 'icon icon-sm')}</button>
      </div>
      <div class="card">
        ${c.equips.length ? c.equips.map(e => filaEquip(e, c.id)).join('')
          : '<div class="card-pad"><p class="ajuda" style="margin:0">Encara no té cap equip.</p></div>'}
      </div>`).join('')}

    <div class="section-title">${icon('users', 'icon icon-sm')} Sense club</div>
    <div class="card">
      ${sense.length ? sense.map(e => filaEquip(e, null)).join('')
        : '<div class="card-pad"><p class="ajuda" style="margin:0">Tots els equips tenen club.</p></div>'}
    </div>`;
}
...
function bloqueUsuaris() {
  ...
  /*
    Esborrar un compte no es desfà, i des d'una llista de dos-cents és
    massa fàcil prémer el que no toca: es pregunta dues vegades i la
    segona demana escriure el correu.
  */
  App.accions.esborrarUsuari = el => {
    const id = Number(el.dataset.usuari);
    const u = usuarisAdmin.find(x => x.id === id);
    if (!u) return;
    const quiEs = `${u.nom} ${u.cognoms || ''}`.trim();

    confirmar({
      titol: `Esborrar el compte de ${quiEs}?`,
      text: 'Perdrà l\'accés i tots els seus permisos. La seva fitxa de jugador i '
          + 'el que hagi jugat es queden. Això no es pot desfer.',
      confirma: 'Continuar',
      perill: true,
      onOk() {
        const ov = $('#overlay');
        ov.innerHTML = `
          <div class="sheet">
            <h3>Segur del tot?</h3>
            <p>Escriu <strong>${esc(u.email || quiEs)}</strong> per confirmar que és aquest compte.</p>
            <div class="field">
              <label for="u-esborrar">Correu del compte</label>
              <input id="u-esborrar" type="text" autocapitalize="off" autocomplete="off" />
            </div>
            <p class="ajuda" id="u-esborrar-error" hidden></p>
            <div class="sheet-actions">
              <button class="btn btn-outline" data-tanca="1">Cancel·lar</button>
              <button class="btn btn-danger" data-fer="1">Esborrar el compte</button>
            </div>
          </div>`;
        ov.hidden = false;
        ov.onclick = async e => {
          if (e.target === ov || e.target.dataset.tanca) { ov.hidden = true; ov.innerHTML = ''; return; }
          if (!e.target.dataset.fer) return;
          const error = $('#u-esborrar-error');
          if ($('#u-esborrar').value.trim().toLowerCase() !== String(u.email || quiEs).toLowerCase()) {
            error.textContent = 'No coincideix.';
            error.hidden = false;
            return;
          }
          e.target.disabled = true;
          try {
            const d = await Store.esborrarUsuari(id);
            usuarisAdmin = d.usuaris;
            ov.hidden = true; ov.innerHTML = '';
            toast('Compte esborrat');
            pintar();
          } catch (err) {
            error.textContent = err.message;
            error.hidden = false;
    ...
    return `
      <div class="fila-usuari">
        <div class="fila-usuari-cap">
          ...
          <button class="icon-btn ghost boto-no" data-accio="esborrarUsuari" data-usuari="${u.id}"
                  aria-label="Esborrar el compte" title="Esborrar el compte">${icon('close')}</button>
        </div>
