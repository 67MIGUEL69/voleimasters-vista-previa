  ...
  if (!grupCalendari) {
    return `
      ...
      <p class="ajuda" style="margin-bottom:10px">
        Tria un grup per posar-li les jornades i els partits.
      </p>
      <!-- Botons de debò: com a files d'una llista no semblava que es
           poguessin tocar. -->
      <div class="botons-grup">
        ${cats.map(c => `
          <button class="boto-grup metall-vora-${c.nivell}" data-accio="obrirCalendari"
                  data-grup="${c.id}">
            <span class="boto-grup-nom">${esc(c.nom)}</span>
            <span class="boto-grup-dada">
              ${(Store.equipsPerCategoria[c.id] || []).length} equips
            </span>
