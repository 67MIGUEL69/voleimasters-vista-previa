...
/**
 * La finestra d'una jornada: del dia que s'obre al dia límit.
 *
 * És el que la coordinació hi posa i el que els equips han de mirar per
 * quedar. Abans hi sortia la data del primer partit de la jornada, que no
 * vol dir res mentre no s'hagin quedat tots.
 */
function finestraJornada(partit) {
  const dia = iso => formatDiaCurt(iso);
  if (partit?.jornada_inici && partit?.jornada_limit) {
    return `Del ${dia(partit.jornada_inici)} al ${dia(partit.jornada_limit)}`;
  }
  if (partit?.jornada_limit) return `Fins al ${dia(partit.jornada_limit)}`;
  if (partit?.jornada_inici) return `Des del ${dia(partit.jornada_inici)}`;
  return formatData(partit?.data);
}

/** «5 oct», sense l'any: dins d'una temporada no cal. */
function formatDiaCurt(iso) {
  if (!iso) return '';
  const d = new Date(String(iso).slice(0, 10) + 'T00:00:00');
  if (isNaN(d)) return String(iso);
  return `${d.getDate()} ${MESOS[d.getMonth()]}`;
}
