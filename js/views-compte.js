...
function viewRegistre() {
  ...
  App.accions = {
    async registrar(boto) {
      const dades = {
        ...
        dni: $('#r-dni').value.trim(),
        telefon: $('#r-telefon').value.trim(),
        data_naixement: $('#r-naixement').value,
      };
      ...
      if (!dades.dni || !dades.telefon || !dades.data_naixement) {
        toast('Falten el DNI, el telèfon o la data de naixement');
        return;
      }
  ...
  return `
    ...
    <div class="card card-pad caixa-estreta">
      ...
      <!-- El DNI, el telèfon i la data de naixement es demanen des del
           primer moment: són el que la lliga necessita per fitxar ningú i
           el que mira la regla de pista. Demanar-los després vol dir anar
           al darrere de la gent. -->
      <div class="fila-camps">
        <div class="field">
          <label for="r-dni">DNI</label>
          <input id="r-dni" type="text" autocapitalize="characters"
                 autocomplete="off" placeholder="12345678Z" />
        </div>
        <div class="field">
          <label for="r-telefon">Telèfon</label>
          <input id="r-telefon" type="tel" inputmode="tel"
                 autocomplete="tel" placeholder="600 000 000" />
        </div>
      </div>

      <div class="field">
        <label for="r-naixement">Data de naixement</label>
        <input id="r-naixement" type="date" autocomplete="bday" />
      </div>
    ...
    <div class="note caixa-estreta" style="margin-top:16px">
      ${icon('info', 'icon icon-sm')}
      <span>El DNI, el telèfon i la data de naixement calen per poder jugar:
      són el que la lliga necessita per fitxar-te. Només els veuen tu i la
      coordinació del teu equip.<br /><br />
      Els permisos de coordinació, entrenador, capità o àrbitre els dona la
      coordinació de la lliga.</span>
    </div>`;
}
