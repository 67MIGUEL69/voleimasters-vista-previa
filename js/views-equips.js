...
function viewPlantilla(id) {
  ...
  const potEntrenador = dades.pot_entrenador;
  ...
  App.accions = {
    /*
      La corona sola no diu què fa, i fer capità algú li dona permisos a
    ...
    */
    capitana(el) {
      ...
      confirmar({
        titol: ho_es ? `Treure la capitania a ${nom}?` : `Fer capità ${nom}?`,
        text: ho_es
          ? 'Deixarà de ser capità i perdrà els permisos de capitania a l\'app '
            + '(convocatòries, amistosos…). Seguirà a la plantilla com a jugador.'
          : 'Sortirà com a capità a la plantilla i a l\'acta, i si té compte podrà fer '
            + 'convocatòries i portar coses de l\'equip a l\'app. Un equip pot tenir més d\'un capità.',
        confirma: ho_es ? 'Treure la capitania' : 'Fer-lo capità',
        perill: ho_es,
        async onOk() {
          try {
            await Store.editarFitxa(id, fitxaId, { es_capita: !ho_es });
            toast(ho_es ? 'Ja no és capità' : 'Ara és capità');
            pintar();
    ...
    /*
      Entrenar i jugar no s'exclouen: qui entrena segueix a la plantilla amb
      el seu dorsal. Com la corona, es pregunta abans: dona permisos.
    */
    entrenador(el) {
      const fitxaId = Number(el.dataset.fitxa);
      const nom = el.dataset.nom;
      const ho_es = el.dataset.entrenador === '1';
      confirmar({
        titol: ho_es ? `Treure ${nom} d'entrenador?` : `Fer entrenador ${nom}?`,
        text: ho_es
          ? 'Deixarà de constar com a entrenador de l\'equip i perdrà els seus permisos. '
            + 'Segueix a la plantilla amb el seu dorsal.'
          : 'Constarà com a entrenador de l\'equip i podrà portar-ne les coses a l\'app '
            + '(convocatòries, capitans, dates…). Segueix jugant amb el seu dorsal.',
        confirma: ho_es ? 'Treure-li' : 'Fer-lo entrenador',
        perill: ho_es,
        async onOk() {
          try {
            await Store.editarFitxa(id, fitxaId, { es_entrenador: !ho_es });
            toast(ho_es ? 'Ja no és entrenador' : 'Ara és entrenador');
            pintar();
          } catch (e) { toast(e.message); }
        },
      });
    ...
    /*
      Qui entrena també pot jugar. A la plantilla només s'hi pot constar
      d'una manera, i constant com a cos tècnic l'àrbitre no el pot
      convocar; això el passa a la llista de jugadors amb el seu dorsal.
    */
    ferJugador(el) {
      const fitxaId = Number(el.dataset.fitxa);
      const nom = el.dataset.nom;
      const ocupats = (dades.plantilla || []).map(f => f.dorsal).filter(d => d !== null);

      const ov = $('#overlay');
      ov.innerHTML = `
        <div class="sheet">
          <h3>${esc(nom)} també juga?</h3>
          <p>Passarà a la llista de jugadors amb aquest dorsal i es podrà convocar
          als partits. Segueix sent del cos tècnic si en té el rol.</p>
          <div class="field">
            <label for="fj-dorsal">Dorsal</label>
            <input id="fj-dorsal" class="camp-codi" type="number" inputmode="numeric"
                   min="0" max="199" placeholder="—" />
          </div>
          <p class="ajuda" id="fj-avis">${ocupats.length
            ? `Ja estan agafats: ${ocupats.join(', ')}.` : 'Encara no hi ha cap dorsal agafat.'}</p>
          <div class="sheet-actions">
            <button class="btn btn-outline" data-tanca="1">Cancel·lar</button>
            <button class="btn btn-primary" data-fer="1">Fer-lo jugador</button>
          </div>
        </div>`;
      ov.hidden = false;
      $('#fj-dorsal').focus();
      ...
      ov.onclick = async ev => {
        if (ev.target === ov || ev.target.dataset.tanca) { ov.hidden = true; ov.innerHTML = ''; return; }
        if (!ev.target.dataset.fer) return;
        const avis = $('#fj-avis');
        const cru = $('#fj-dorsal').value.trim();
        ...
        if (ocupats.includes(dorsal)) {
          avis.textContent = `El ${dorsal} ja és d'un altre jugador.`;
          return;
        }
        ev.target.disabled = true;
        try {
          await Store.editarFitxa(id, fitxaId, { tipus: 'jugador', dorsal });
          ov.hidden = true; ov.innerHTML = '';
          toast(`${nom} ja pot jugar`);
          pintar();
        } catch (e) {
  ...
  const fila = f => `
    <div class="fila-fitxa">
      ...
      <button class="fitxa-qui" data-anar="#/persona/${f.persona_id}">
        ...
        <div class="ajuda">
          ${f.es_capita ? `<span class="marca-capita">${icon('corona', 'icon icon-xs')} Capità</span> · ` : ''}${f.es_entrenador && f.tipus === 'jugador' ? `<span class="marca-entrenador">${icon('whistle', 'icon icon-xs')} Entrenador</span> · ` : ''}${esc(NOM_TIPUS[f.tipus] || f.tipus)}
          ${f.data_naixement ? ' · ' + esc(f.data_naixement) : ''}
        </div>
      </button>
      ${potGestionar || potCapita || potEntrenador ? `
        <div class="fitxa-accions">
          ${potEntrenador && f.tipus === 'jugador' ? `
            <button class="icon-btn ghost boto-xiulet ${f.es_entrenador ? 'actiu' : ''}" data-accio="entrenador"
                    data-fitxa="${f.fitxa_id}" data-nom="${esc(f.nom)}" data-entrenador="${f.es_entrenador ? 1 : 0}"
                    aria-label="${f.es_entrenador ? 'Treure d\'entrenador' : 'Fer entrenador'}"
                    title="${f.es_entrenador ? 'Treure d\'entrenador' : 'Fer entrenador'}">${icon('whistle')}</button>` : ''}
          ${potGestionar && f.tipus !== 'jugador' ? `
            <button class="icon-btn ghost" data-accio="ferJugador"
                    data-fitxa="${f.fitxa_id}" data-nom="${esc(f.nom)}"
                    aria-label="Fer-lo jugador" title="També juga">${icon('ball')}</button>` : ''}
          ${potCapita && f.tipus === 'jugador' ? `
            <button class="icon-btn ghost boto-corona ${f.es_capita ? 'actiu' : ''}" data-accio="capitana"
                    ...
                    aria-label="${f.es_capita ? 'Treure la capitania' : 'Fer capità'}"
                    title="${f.es_capita ? 'Treure la capitania' : 'Fer capità'}">${icon('corona')}</button>` : ''}
          ${potGestionar ? `
  ...
  return `
    <div class="page-head">
      ...
      <p class="page-sub">
        ${jugadores.length} jugadors${tecnics.length ? ` · ${tecnics.length} al cos tècnic` : ''}
      </p>
    </div>
    ...
    <div class="section-title">${icon('users', 'icon icon-sm')} Jugadors</div>
    <div class="card">
...
/* ---------- Posar un àrbitre ----------
   Qui porta un equip sap qui li xiularà els partits abans que la lliga.
   Amb el correu n'hi ha prou: si ja té compte, li activa el perfil
   d'àrbitre; si no, li arriba un correu per registrar-se i el perfil
   l'espera. */

function bloqueArbitres(equipId) {
  const dades = Store.plantilles[equipId];
  if (!dades?.pot_gestionar) return '';

  App.accions.convidarArbitre = async boto => {
    const camp = $('#a-email');
    const email = camp.value.trim();
    if (!email) { toast('Escriu el correu de l\'àrbitre'); return; }

    boto.disabled = true;
    boto.textContent = 'Enviant…';
    try {
      const d = await Store.convidarArbitre(equipId, email);
      camp.value = '';
      toast(d.activat
        ? 'Ja té el perfil d\'àrbitre activat'
        : (d.correu ? 'Convidat: li arribarà un correu' : 'Convidat, però el correu no ha sortit'));
    } catch (e) {
      toast(e.message);
    }
    boto.disabled = false;
    boto.textContent = 'Posar-lo d\'àrbitre';
  };

  return `
    <div class="section-title">${icon('whistle', 'icon icon-sm')} Àrbitres</div>
    <div class="card card-pad">
      <div class="field">
        <label for="a-email">Correu de l'àrbitre</label>
        <input id="a-email" type="email" autocapitalize="off" placeholder="nom@exemple.cat" />
      </div>
      <button class="btn btn-outline btn-block" data-accio="convidarArbitre">
        Posar-lo d'àrbitre
      </button>
      <p class="nota-petita">
        Si ja té compte, li queda activat de seguida. Si no en té, li arriba un correu
        per registrar-se i entrarà ja com a àrbitre. Podrà arbitrar qualsevol partit
        de la lliga que li assignin.
      </p>
    </div>`;
}
...
function bloqueCrearEquip() {
  ...
  App.accions.crearEquip = async boto => {
    ...
    // El grup és optatiu: sense ell, l'equip queda a l'espera que la
    // coordinació de la lliga l'hi posi. Amb tots els grups plens, exigir-lo
    // volia dir que un club no pogués donar d'alta cap equip.
    const grup = Number($('#e-categoria').value);
    if (grup) dades.categoria_id = grup;
    ...
    <div class="card card-pad" style="margin-bottom:16px">
      ...
      <div class="field">
        ...
        <select id="e-categoria">
          <option value="">— Encara sense grup —</option>
      ...
      <p class="ajuda" style="margin:-4px 0 12px">
        Si encara no saps a quin grup anirà, deixa-ho sense grup: la coordinació
        de la lliga l'hi posarà.
      </p>
      <div class="field">
  ...
  const bloc = costat => {
    ...
    if (!d.pot_convocar) {
      ...
      const buit = !d.visible
        ? (d.convocats
            ? `Ja tenen ${d.convocats} convocats. Els noms surten a l'acta
            ...
            ${convocats.length
              ? convocats.map(f => `
                  <div class="log-item">
                    ...
                    ${f.es_capita ? '<span class="log-time">capità</span>' : ''}
                  </div>`).join('')
      ...
      <label class="fila-convocada">
        ...
        <span class="conv-qui">
          ...
          ${f.es_capita || f.tipus !== 'jugador' ? `<span class="ajuda">
            ${f.es_capita ? 'Capità' : ''}${f.es_capita && f.tipus !== 'jugador' ? ' · ' : ''}
...
function pintarComptador(equipId) {
  ...
  if (on) on.textContent = `${marcades} convocats`;
}
  ...
  Object.assign(App.accions, {
    ...
    fitxarSolicitud(el) {
      ...
      ov.onclick = async e => {
        ...
        if (ocupats.includes(dorsal)) {
          avis.textContent = `El ${dorsal} ja és d'un altre jugador. Ja estan agafats: ${ocupats.join(', ')}.`;
          return;
        }
