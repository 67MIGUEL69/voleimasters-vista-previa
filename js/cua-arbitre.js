...
const MotorMarcador = {
  ...
  formacio(m, cos, ctx) {
    ...
    for (const costat of ['local', 'visitant']) {
      ...
      if (dorsals.length !== 6) throw new ErrorApi("A pista hi ha d'haver 6 jugadores.", 422);
  ...
  canvi(m, cos, ctx) {
    ...
    if (surt === entra) throw new ErrorApi('Has triat la mateixa jugadora dues vegades.', 422);
